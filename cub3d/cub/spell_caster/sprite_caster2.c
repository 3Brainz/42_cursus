...
void		sprite_ver_line(int stripe, t_game_v *game_v, t_data *img, t_window *window)
{
	...
	while (y < s_caster->draw_end_y)
	{
		...
		s_caster->tex_y = ((s_caster->d * window->textuures->sprite_texture->img_height) / s_caster->sprite_height) / 256;
		color = (unsigned int)window->textuures->sprite_texture->addr[window->textuures->sprite_texture->img_width * s_caster->tex_y + s_caster->tex_x];
...
void		sprite_print(t_player *player, t_game_v *game_v, t_data *img, t_window *window)
{
	...
	{
		s_caster->tex_x = (int)(256 * (stripe - (-s_caster->sprite_width / 2 + s_caster->sprite_screen_x)) * window->textuures->sprite_texture->img_width / s_caster->sprite_width) / 256;
